...
//	}
	...
	public static void autoSizeColumn(Sheet sheet, int cols) {
		...
		try {
			for (short k = 0; k < cols; k++) {
				if(MIN_COLUMN_WIDTH > sheet.getColumnWidth(k)){
					sheet.setColumnWidth(k, MIN_COLUMN_WIDTH);
				}
			}
		} catch (Throwable e) {
		}
	...
	public static CellStyle getHeadStyle(Workbook wb)
 	{
 		//CellStyle headStyle = getStyle(wb, HSSFColor.BLACK.index, getRGBColor(palette, 204, 255, 204), CellStyle.ALIGN_CENTER);
	...
	public static CellStyle getDataStyle(Workbook wb, DataFormat dataFormat, String align, String format){
 		//CellStyle style = ExcelUtil.getStyle(wb, HSSFColor.BLACK.index, ExcelUtil.getRGBColor(palette, 153, 204, 255), CellStyle.ALIGN_LEFT);
	...
	/**
	 * 根据目标类型，获取单元格的内容
	 * 只支持String、int、long、boolean、double几种类型
	 * @param cell
	 * @param cls
	 * @return
	 */
			...
			switch(cType){
				...
				default:
					...
					Boolean bsv = !(null == sv 
							...
							|| "0".equals(sv.trim())
							|| "0.0".equals(sv.trim())
							|| "false".equalsIgnoreCase(sv.trim()));
					return (E)bsv;
			}
