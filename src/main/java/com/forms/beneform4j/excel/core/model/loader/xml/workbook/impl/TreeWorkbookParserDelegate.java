...
public class TreeWorkbookParserDelegate {
    ...
    public static TreeEM parseWorkbookDocument(String modelId, Document document) {
        ...
        if (!XmlEMLoader.TREE_WORKBOOK_ELEMENT_NAME.equalsIgnoreCase(name)) {
            workbook = XmlHelper.getChildElementByTagName(element, XmlEMLoader.TREE_WORKBOOK_ELEMENT_NAME);
        }
        if (null != workbook) {
    ...
    public static TreeEM parseWorkbookElement(String modelId, Element workbook) {
        ...
        List<Element> regions = XmlHelper.getChildElementsByTagName(workbook, XmlEMLoader.REGION_ELEMENT_NAME);
        ...
        ITreeEMComponentParser parser = ExcelComponentConfig.getTreeEMComponentParser(XmlEMLoader.GRID_COMPONENT_TYPE);
        if (null != parser) {
    ...
    private static TreeEMSheet parseSheetElement(String modelId, Element sheet) {
        List<Element> regions = XmlHelper.getChildElementsByTagName(sheet, XmlEMLoader.REGION_ELEMENT_NAME);
    ...
    private static String getParserType(Element region) {
        ...
        if (CoreUtils.isBlank(type)) {
            ...
            if (null != firstChild && XmlEMLoader.REGION_ELEMENT_NAME.equals(XmlHelper.getLocalName(firstChild))) {
                type = XmlEMLoader.NESTED_REGION_COMPONENT_TYPE;
            } else {
                type = XmlEMLoader.GRID_COMPONENT_TYPE;
            }
        }
        return type;
    }
